...
// Get available slots for a profile on a date
router.get('/slots/:profileId/:date', (req: Request, res: Response) => {
  const { profileId, date } = req.params;
  const profile = getOne('SELECT * FROM profiles WHERE id = ?', profileId);
  if (!profile) return res.status(404).json({ error: 'Profile not found' });

  const dayOfWeek = new Date(date).getDay();
  const availability = getAll(
    'SELECT * FROM availability WHERE profile_id = ? AND day_of_week = ? AND is_active = 1',
    profileId, dayOfWeek);
  ...
  for (const b of booked) {
    for (let h = 0; h < b.duration_hours; h++) bookedHours.add(b.start_hour + h);
  }
...
router.post('/', authRequired, (req: AuthRequest, res: Response) => {
  ...
  const profile = getOne('SELECT * FROM profiles WHERE id = ?', profile_id);
  if (!profile) return res.status(404).json({ error: 'Profile not found' });
...
router.get('/professional', authRequired, (req: AuthRequest, res: Response) => {
  const profile = getOne('SELECT id FROM profiles WHERE user_id = ?', req.userId!);
...
// Cancel booking
router.post('/:id/cancel', authRequired, (req: AuthRequest, res: Response) => {
  const booking = getOne('SELECT * FROM bookings WHERE id = ? AND client_id = ?', req.params.id, req.userId!);
  ...
  if (hoursUntilSlot > 2) {
    refundAmount = booking.booking_amount;
    refundType = 'full';
  } else {
    refundAmount = booking.booking_amount * 0.25;
    refundType = 'partial';
  }

  run('UPDATE bookings SET status = ? WHERE id = ?', 'cancelled', req.params.id);
...
router.put('/:id', authRequired, (req: AuthRequest, res: Response) => {
  ...
  run('UPDATE bookings SET booking_date = ?, start_hour = ? WHERE id = ?', newDate, newHour, req.params.id);
  res.json({ message: 'Booking updated' });
});

// Mark booking complete (professional)
router.post('/:id/complete', authRequired, (req: AuthRequest, res: Response) => {
  const profile = getOne('SELECT id FROM profiles WHERE user_id = ?', req.userId!);
  if (!profile) return res.status(403).json({ error: 'Not a professional' });

  const booking = getOne('SELECT * FROM bookings WHERE id = ? AND profile_id = ?', req.params.id, profile.id);
  if (!booking) return res.status(404).json({ error: 'Booking not found' });
