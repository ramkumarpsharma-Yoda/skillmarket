...
router.get('/', optionalAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (search) { sql += ` AND (p.expertise ILIKE ? OR u.name ILIKE ?)`; params.push(`%${search}%`, `%${search}%`); }

    const offset = (parseInt(page as string) - 1) * parseInt(limit as string);
    sql += ` ORDER BY avg_rating DESC LIMIT ? OFFSET ?`;
    params.push(parseInt(limit as string), offset);

    const profiles = await getAll(sql, ...params);
    const total = await getOne('SELECT COUNT(*) as count FROM profiles WHERE status = ?', 'approved');
...
router.get('/:id', optionalAuth, async (req: Request, res: Response) => {
  try {
    ...
    const availability = await getAll('SELECT * FROM availability WHERE profile_id = ? AND is_active = 1', req.params.id);
...
router.post('/', authRequired, upload.fields([
  ...
  try {
    ...
    const existing = await getOne('SELECT id FROM profiles WHERE user_id = ?', req.userId!);
    ...
    await run(`INSERT INTO profiles (id, user_id, category, expertise, years_experience, qualifications, association, bio,
      base_charge, min_hours, max_hours, consultation_modes,
    ...
    await run('UPDATE users SET role = ? WHERE id = ?', 'professional', req.userId!);

    if (req.body.availability) {
      const avail = JSON.parse(req.body.availability);
      for (const slot of avail) {
        await run('INSERT INTO availability (id, profile_id, day_of_week, start_hour, end_hour) VALUES (?, ?, ?, ?, ?)',
...
router.put('/:id', authRequired, async (req: AuthRequest, res: Response) => {
  try {
    const profile = await getOne('SELECT * FROM profiles WHERE id = ? AND user_id = ?', req.params.id, req.userId!);
    ...
    await run(`UPDATE profiles SET base_charge = COALESCE(?, base_charge), min_hours = COALESCE(?, min_hours),
